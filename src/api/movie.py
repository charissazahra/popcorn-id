...
from src.models.movie import Movie, MovieCreate
...
def add_movie(movie_create: MovieCreate):  # ganti parameter
    ...
    if not file_path.exists():
        raise FileNotFoundError(
            "File movie_data tidak ditemukan. Pastikan sudah dibuat secara manual dulu"
        )
    ...
    if not data:  # krn data itu list dictionary jdi pke not
        ...
        last_id = data[-1][
            "id"
        ]  # dapetin last data pada list, tapi kita cuma dapetin id nya aja dari dict
...
def get_movie(movie_id: int):  # ganti parameter
    ...
    for movie in data:  # looping movie di dalam list of dict
        if (
            movie["id"] == movie_id
        ):  # dari id yg ada di list of dict dan berdasarkan id yg diminta
            return movie
