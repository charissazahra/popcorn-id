...
def test_add_movie_valid():
    ...
    response = client.get(f"/movies/{movie_id}") #Kirim request GET ke /movies/{parameter}
    assert response.status_code == 200 #Pastikan responnya sukses (kode 200)
    ...
    assert response.status_code in [404, 422] #(kode 404 atau 422 not found or rejected)

    # GET movie dengan Non-Integer ID
    response = client.get(f"/movies/abc")
    assert response.status_code == 422 #(kode 422 Unprocessable Entity, id hrs integer)
